...
export class UserDB {
  static registerUserDB = async (cpf: string, name: string, email: string) => {
    const verify = await getConnection().raw('SELECT cpf,email FROM users WHERE cpf=? OR email=? LIMIT 1', [cpf, email]);
    ...
    const response = await getConnection().raw('INSERT INTO users (cpf,name,email) VALUES (?,?,?) RETURNING *;', [cpf, name, email]);
    return response.rows[0];
  };
  static userName = async (id: string) => {
    const verify = await getConnection().raw('SELECT name FROM users WHERE id=? LIMIT 1', [id]);
